...
// Plugin customizado para rodar a API de e-mail localmente no Vite
const localSmtpPlugin = (env: Record<string, string>) => ({
  name: 'local-smtp-api',
        ...
        req.on('end', async () => {
          try {
            ...
            const mailOptions = {
              from: env.SMTP_FROM,
              to: env.SMTP_TO,
              subject: `Novo Lead Dominus: ${data.name}`,
              html: `
...
export default defineConfig(({ mode }) => {
  // Carrega as variáveis do .env (ignorando prefixos)
  const env = loadEnv(mode, process.cwd(), '');
  
  return {
    plugins: [react(), localSmtpPlugin(env)],
    server: {
      host: true, // Expõe o servidor para a rede local (0.0.0.0)
      allowedHosts: true, // Permite conexões de qualquer host (como o ngrok)
    },
    build: {
      target: 'esnext',
      minify: 'esbuild',
